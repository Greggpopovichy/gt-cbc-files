...
const princessNames = princesses.map(function(princess){
  return princess.names;
});
...
const filteredPrincesses = princesses.filter(function(princess){
  return princess.name.startsWith('M');
});
console.log(filteredPrincesses);
...
const sumOfAges = princesses.reduce(function(result, princess){
  return result + princess.age;
}, 0);

const avg = sumOfAges / princesses.length;
console.log(avg);
